...
// Given "cbacdcbc"
// Return "acdb"
public // Given a string which contains only lowercase letters, remove duplicate letters
// so that every letter appear once and only once. You must make sure your result is the
// smallest in lexicographical order among all possible results.
//
// Example:
...
public class Solution {
  public String removeDuplicateLetters(String s) {
    ...
    for (int i = 0; i < s.length(); i++){
      ...
      while (stack.isEmpty()==false&& stack.peek() > c && count[stack.peek()-'a']>0){
        visit[stack.pop()-'a'] = false;
      }
      stack.push(c);visit[c-'a'] = true;
    }
